...
def auction_contract(w3, get_contract, auction_start):
    with open(f"{HOME_DIR}/contract/simple_open_auction.vy") as f:
...
def test_initial_state(w3, tester, auction_contract, auction_start):
    ...
    real_beneficiary = auction_contract.beneficiary()
    real_val = w3.eth.accounts[0]
    assert real_beneficiary == real_val
